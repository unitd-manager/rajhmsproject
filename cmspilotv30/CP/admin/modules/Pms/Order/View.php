<?
class CP_Admin_Modules_Pms_Order_View extends CP_Common_Lib_ModuleViewAbstract
{
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            /*if($cpCfg['m.pms.ecommerce.order.orderAmountForPvt'] == 1){
                $SQLPvt = "
                SELECT oi.*
                      ,o.order_id
                      ,o.contact_module
    ...
    function getRightPanel($row){
        ...
        $links = '';
        $paymentLink = '';
        //{$displayLinkData->getLinkPortalMain('pms_order', 'ecommerce_orderItemLink', 'Order Items', $row)}
        //$paymentLink = $displayLinkData->getLinkPortalMain('pms_order', 'pms_paymentLink', 'Payments Linked', $row);

        // This is used to display Credit Note Portal Display       
        $creditNote = '';
        if ($cpCfg['m.pms.order.hasCreditNoteLink']){
            $creditNote = $this->getCreditNotePortalDisplay($row);
        }

        // This is used to display Misc Receipt Portal Display       
        $miscReceipt = '';
        if ($cpCfg['m.pms.order.hasMiscReceiptForPvt']){
        ...
        $bookReceipt = '';
        if ($cpCfg['m.pms.order.hasBookReceipt']){
            $bookReceipt = $this->getBookReceiptPortalDisplay($row);
        }
        $bookReceipt = '';

        // This is used to display Insurance Link Portal
        if($cpCfg['m.pms.order.hasInsuranceLink'] && $rowCourse['course_type'] != 'Short Term'){
            $links .= $displayLinkData->getLinkPortalMain("pms_order", "pms_insuranceLink", "Insurance Linked", $row);
        }
    ...
    function getOrderItemPortalDisplay($row){
        ...
        while ($rowOrderItem = $db->sql_fetchrow($result)) {
            ...
            if ($cpCfg['m.pms.order.hasCheckBoxForInvoiceItem']) {
            ...
            if ($cpCfg['m.pms.ecommerce.order.orderItemDisplayForPvt'] == false){
                $rows .= "
                <tr>
                    {$tdCheckBox}
                    <td>{$contactName}</td>
        ...
        if($cpCfg['m.pms.ecommerce.order.orderItemDisplayForPvt'] == 1){
            ...
            $expCourse   = array('condn' => " AND module='pms_course'");
        ...
        $formAction = "index.php?module=pms_order&_spAction=generateInvoice&showHTML=0";
        $formActionPvt = "index.php?_topRm=finance&module=pms_order&_spAction=generateInvoiceFormPvt&showHTML=0&order_id={$row['order_id']}";

        $recordCount = $fn->getRecordCount('invoice', "order_id = {$row['order_id']}");
        // This is used to display Invoice for Pvt : Used for Mass IMS
        if($cpCfg['m.pms.ecommerce.order.invoiceForPvt']) {
            if ($recordCount > 0){
                $formAction = '';
        ...
        } else if ($cpCfg['m.pms.ecommerce.order.orderItemDisplayForEnt'] == true) {
            $formAction = "index.php?_topRm=finance&module=pms_order&_spAction=generateMonthlyInvoiceForEntForm&showHTML=0&order_id={$row['order_id']}";
            $invoiceBtn = "
        ...
        <tr class=''>
        <td>
            <div id='pms_company#pms_orderLink' class=''>
    ...
    function getInvoicePortalDisplay($row){
        ...
        if ($cpCfg['m.pms.ecommerce.order.invoiceForPvt'] == true){
            $SQL = "
            ...
            ";
        } else if ($cpCfg['m.pms.order.invoiceForEnt']){
            $SQL = "
            SELECT i.*
                   ,(
                   SELECT GROUP_CONCAT(r.receipt_code ORDER BY r.receipt_code SEPARATOR ', ')
        ...
        while ($rowInvoice = $db->sql_fetchrow($result)) {
            ...
            if ($row['contact_type'] == 'Indvidual') {
                $urlPrint = "index.php?_topRm=finance&module=pms_order&_spAction=printInvoiceIndividual&invoice_code={$rowInvoice['invoice_code']}&showHTML=0";
            } else {
                $urlPrint = "index.php?_topRm=finance&module=pms_order&_spAction=printInvoice&invoice_code={$rowInvoice['invoice_code']}&showHTML=0";
            }
            ...
            if($invoice_code == '' || $invoice_code != $rowInvoice['invoice_code']){
                ...
                $editRow = '<td></td>';
                if ($cpCfg['m.pms.order.hasEditInvoiceForPvt']){
                    if ($rowInvoice['status'] == 'Due' 
                     || $rowInvoice['status'] == '' 
                     || $rowInvoice['status'] == 'Partial Payment'
                    ) {
                        $editURL = "index.php?_topRm=finance&module=pms_order&_spAction=editInvoiceFormPvt&showHTML=0&invoice_id={$rowInvoice['invoice_id']}&order_id={$row['order_id']}";
                ...
                $deleteRow = '';
                if ($cpCfg['m.pms.order.hasDeleteInvoiceForPvt']){
                    $deleteURL = "index.php?_topRm=finance&module=pms_order&_spAction=deleteInvoiceFormPvt&showHTML=0&invoice_id={$rowInvoice['invoice_id']}";
                    $deleteRow = "<td><a href='{$deleteURL}' id='deleteInvoicePvt' invoice_id='{$rowInvoice['invoice_id']}'>Delete</a></td>";
                }

                $month = '';
                $tdRow = '';
                if ($cpCfg['m.pms.order.invoiceForEnt']) {
            ...
            if ($cpCfg['m.pms.ecommerce.order.invoiceForPvt'] == 1){
        ...
        $formActionCreditNote = "index.php?_topRm=finance&module=pms_order&_spAction=generateCreditNoteForm&showHTML=0&order_id={$row['order_id']}";

        $creditNote = '';
        if ($cpCfg['m.pms.order.hasCreditNoteLink']){
            $formAction = "index.php?_topRm=finance&module=pms_order&_spAction=generateReceiptFormPvt&showHTML=0&order_id={$row['order_id']}";
            $creditNote = "
        ...
        $miscReceiptBtn = '';
        if ($cpCfg['m.pms.order.hasMiscReceipt']){
            $miscFormAction = "index.php?_topRm=finance&module=pms_order&_spAction=generateMiscReceiptFormPvt&showHTML=0&order_id={$row['order_id']}";
        ...
        $bookReceiptBtn = '';
        if ($cpCfg['m.pms.order.hasBookReceipt']){
            $bookFormAction = "index.php?_topRm=finance&module=pms_order&_spAction=generateBookReceiptForm&showHTML=0&order_id={$row['order_id']}";
        ...
        if($cpCfg['m.pms.ecommerce.order.receiptForPvt']) {
        ...
        } else if ($cpCfg['m.pms.order.receiptForEnt']) {
            $formAction = "index.php?_topRm=finance&module=pms_order&_spAction=generateReceiptForEntForm&showHTML=0&order_id={$row['order_id']}";
            $receiptBtn = "
            <button href='{$formAction}' id='generateReceiptEnt' 
            class='button mt5 ml5 mb20'>Generate Receipt</button>
            ";
        } else {
            $formAction = "index.php?_topRm=finance&module=pms_order&_spAction=generateReceiptForm&showHTML=0&order_id={$row['order_id']}";
            $receiptBtn = "
    ...
    function getInvoiceRecords($row){
        ...
        while ($rowInvoice = $db->sql_fetchrow($result)) {
            ...
            if($invoice_code == '' || $invoice_code != $rowInvoice['invoice_code']){
                ...
                $cancelInvoiceLink = '';
                if ($rowInvoice['status'] != 'Cancelled') {
                    $cancelInvoiceLink = "<a href='#' class='cancelInvoice' invoice_code={$rowInvoice['invoice_id']}>Cancel Invoice</a>";
                }

                $reportLink = "index.php?module=pms_order&_spAction=printInvoiceInFpdf&record_id={$rowInvoice['invoice_id']}&showHTML=0";
    ...
    function getCreditNotePortalDisplay($row){
        ...
        while ($rowCredit = $db->sql_fetchrow($result)) {
            ...
            $mediaLink = "index.php?plugin=common_media&_spAction=saveMedia&room=pms_creditNote&recordType=attachment&media_id={$mediaRec['media_id']}&showHTML=0";
    ...
    function getReceiptPortalDisplay($row){
        ...
        while ($rowReceipt = $db->sql_fetchrow($result)) {
            ...
            if ($row['contact_type'] == 'Indvidual') {
                $urlPrint = "index.php?_topRm=finance&module=pms_order&_spAction=printReceiptIndividual&receipt_code={$rowReceipt['receipt_code']}&showHTML=0";
            } else {
            ...
            $editRow = '';
            if ($cpCfg['m.pms.order.hasEditReceiptForPvt'] && $count == $numRows){
                $editURL = "index.php?_topRm=finance&module=pms_order&_spAction=editReceiptFormPvt&receipt_id={$rowReceipt['receipt_id']}&order_id={$row['order_id']}&showHTML=0";
            ...
            if ($rowReceipt['receipt_status'] != 'Cancelled') {
                $cancelReceiptLink = "<a href='#' class='cancelReceipt' receipt_code={$rowReceipt['receipt_id']}>Cancel Receipt</a>";
            }
            
            $reportLink = "index.php?module=pms_order&_spAction=printReceiptInFpdf&record_id={$rowReceipt['receipt_id']}&showHTML=0";
        ...
        $editHeader = '';
        if ($cpCfg['m.pms.order.hasEditReceiptForPvt']){
            $editHeader = "<th>Edit</th>";
        }
        ...
        $refundButton = '';
        if ($cpCfg['m.pms.order.hasRefund']){
            $refundButton = "
        ...
        <tr class=''>
        <td>
            <div id='receiptPortalOuter' class='linkPortalWrapper pms_company__pms_orderLink'>
            <h2>Receipt(s)</h2>
    ...
    function getRefundPortalDisplay($row){
        ...
        while ($rowRefund = $db->sql_fetchrow($result)) {
            ...
            if ($row['contact_type'] == 'Indvidual') {
                $urlPrint = "index.php?_topRm=finance&module=pms_order&_spAction=printRefundIndividual&refund_code={$rowRefund['refund_code']}&showHTML=0";
            } else {
                $urlPrint = "index.php?_topRm=finance&module=pms_order&_spAction=printRefund&refund_code={$rowRefund['refund_code']}&showHTML=0";
            }

            $expMedia = array('condn' => " AND media_type = 'attachment' AND room_name = 'pms_refund' AND actual_file_name LIKE '%{$rowRefund['refund_code']}%'");
            $mediaRec = $fn->getRecordRowByID('media', 'record_id', $rowRefund['refund_id'], $expMedia);
            $mediaLink = "index.php?plugin=common_media&_spAction=saveMedia&room=pms_refund&recordType=attachment&media_id={$mediaRec['media_id']}&showHTML=0";
        ...
        $formAction = "index.php?_topRm=finance&module=pms_order&_spAction=generateRefundForm&showHTML=0&order_id={$row['order_id']}";
    ...
    function getTotalForPvtInst($result, $getTotalOnly = ''){
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            if($course_type == 'Long Term' && $row['module'] == 'pms_subject'){
            ...
            else if($course_type == 'Short Term' && $row['module'] == 'pms_course'){
                $total = $row['unit_price'];
            }
     ...
     */
     function getGenerateReceiptForm() {
        ...
        $formAction = "index.php?_topRm=finance&module=pms_order&_spAction=generateReceiptFormSubmit&showHTML=0";
     ...
     *
     */
    function getGenerateReceiptForEntFormOld() {
        /********************************* PROCESS ************************************/
        /*
     ...
     */
     function getGenerateMonthlyInvoiceForEntFormOld() {
        ...
        $formAction = "index.php?_topRm=finance&module=pms_order&_spAction=generateMonthlyInvoiceForEntFormSubmit&showHTML=0";
     ...
     function getGenerateMonthlyInvoiceForEntForm() {
        ...
        while ($rowOI = $db->sql_fetchrow($resultOI)) {
            ...
            for($i=1;$i<=12;$i++){
                ...
                if ($order_status != '' 
                    && (($contactRec['status'] == 'Active' && $order_status != 'Cancelled')
                        ||($contactRec['status'] == 'Applied for Withdrawal'))
                    ) {
                    $status = $order_status;
                } else {
     ...
     */
     function getGenerateCreditNoteForm() {
        ...
        $formAction = "index.php?_topRm=finance&module=pms_order&_spAction=generateCreditNoteFormSubmit&showHTML=0";
     ...
     */
     function getGenerateRefundForm() {
        ...
        $formAction = "index.php?_topRm=finance&module=pms_order&_spAction=generateRefundFormSubmit&showHTML=0";
     ...
     *
     */
     function getGenerateReceiptFormPvtOld() {
        $ln = Zend_Registry::get('ln');
        $fn = Zend_Registry::get('fn');
        $cpCfg = Zend_Registry::get('cpCfg');
        $tv = Zend_Registry::get('tv');
        ...
        $formAction = "index.php?_topRm=finance&module=pms_order&_spAction=generateReceiptFormSubmitPvt&showHTML=0";
     ...
     */
     function getEditInvoiceFormPvt() {
        ...
        $expEdit = array('isEditable' => 0);
        $formAction = "index.php?_topRm=finance&module=pms_order&_spAction=editInvoiceFormSubmitPvt&showHTML=0";
        $text = "
     ...
     */
     function getEditReceiptFormPvt() {
        ...
        $formAction = "index.php?_topRm=finance&module=pms_order&_spAction=editReceiptFormSubmitPvt&showHTML=0";
     ...
     */
     function getGenerateBookReceiptForm() {
        ...
        $formAction = "index.php?_topRm=finance&module=pms_order&_spAction=generateBookReceiptFormSubmit&showHTML=0";
