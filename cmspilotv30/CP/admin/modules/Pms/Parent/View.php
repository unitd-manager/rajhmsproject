<?
class CP_Admin_Modules_Pms_Parent_View extends CP_Common_Modules_Pms_Parent_View
{
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            if($cpCfg['m.pms.parent.hasSalutation'] == 1){
                $rowsSalutation = "
    ...
    function getRightPanel($row){
        ...
        if ($cpCfg['m.pms.parent.hasStudentEnrollment']) {
            $rows .= $this->getCourseStudentDisplay($row);
        }

        $formAction = "index.php?_topRm=main&module=pms_parent&_spAction=parentTransferForm&showHTML=0&parent_id={$record_id}";
        $text = "
        <div>
            <a href='{$formAction}' id='parentTransfer' class='button mt5 ml5 mb10' parent_id={$record_id}>Transfer to other Branch</a>
        </div>
        {$media->getRightPanelMediaDisplay('Picture', 'pms_parent', 'picture', $row)}
        {$displayLinkData->getLinkPortalMain('pms_parent', 'pms_contactLink', 'Student Linked', $row)}
        {$rows}
    ...
    function getQuickSearch() {
        ...
        <div class='helpContentButton'>
            <a class='helpContentTask button' module_name='{$module_title}' href='#'>Help</a>
        </div>
        ";
    ...
    function getImportInstructions() {
        ...
        $url = 'index.php?_spAction=streamFile&showHTML=0&modname=pms_parent&filename=parent-import-template.xls';
        $text = "
    ...
    function getCourseStudentDisplay($row){
        ...
        while ($rowCC = $db->sql_fetchrow($result)) {
            if ($order_id != $rowCC['order_id']){
                $printUrl    = "index.php?module=pms_company&_spAction=printVoucher&id={$row['company_id']}&order_id={$rowCC['order_id']}&showHTML=0";
                $orderUrl = "/admin/index.php?_topRm=finance&module=pms_order&_action=detail&order_id={$rowCC['order_id']}";
                $editurl = "index.php?_topRm=main&_spAction=bulkParentStudentEnrollment&module=pms_orderLink&srcRoom=pms_company&lnkRoom=pms_orderLink&parent_id={$row['parent_id']}&order_id={$rowCC['order_id']}&showHTML=0
                ";
                ///admin/index.php?_spAction=edit&srcRoom=pms_company&lnkRoom=pms_orderLink&id={$rowCC['order_id']}&showHTML=0
                ...
                <tr style='background-color: #F4F4F4; color:#000000;'>
                    ...
                    <td class='portalActBtns'>
                        <div style='float:right'>
                            ...
                            <a class='deletePortalRecord' srcroomid={$row['parent_id']} link='/admin/index.php?_spAction=deletePortalRecordByID&srcRoom=pms_parent&lnkRoom=pms_orderLink&id={$rowCC['order_id']}&showHTML=0' href='javascript:void(0);'>
            ...
            $contact_link = "/admin/index.php?_topRm=main&module=pms_contact&_action=detail&record_id={$rowCC['contact_id']}";
        ...
        <th>Date</th>
        <th>Student</th>
        <th>{$modulesArr['pms_course']['title']}</th>
        <th>{$modulesArr['pms_level']['title']}</th>
        <th>{$modulesArr['pms_batch']['title']}</th>
        <th>Subsidy</th>
        <th>Enrollment Year</th>
        ";

        $text = "
        <div class='linkPortalWrapper pms_parent__pms_orderLink' id='pms_parent#pms_orderLink'>
            <div expanded='1' class='header'>
                <div class='linkPortalDataWrapper'>
                    <div class='actBtns'>
                        <a id='bulkParentCourseLink' class='' dialogtitle='Bulk Parent Contact Link' href='index.php?_topRm=main&_spAction=bulkParentStudentEnrollment&module=pms_orderLink&srcRoom=pms_parent&lnkRoom=pms_orderLink&parent_id={$row['parent_id']}&showHTML=0';'> 
     ...
     */
     function getParentTransferForm() {
        ...
        $formAction = "index.php?_topRm=main&module=pms_parent&_spAction=parentTransferFormSubmitLatest&showHTML=0";
        ...
        if ($cpCfg['m.pms.batch.multiSiteBranch'] == 1) {
            ...
            WHERE cc.parent_id = {$parent_id}
              ...
              AND cc.site_id = {$site_id}
            ORDER BY cc.year_of_enrollment ASC, cc.contact_id ASC
            ";
            $resultCC = $db->sql_query($sqlCC);
            $rows = '';
            $s = array();
            while ($rowCC = $db->sql_fetchrow($resultCC)) {
                $studentRec = $fn->getRecordRowById('contact', 'contact_id', $rowCC['contact_id']);
                $courseRec  = $fn->getRecordRowById('course', 'course_id', $rowCC['course_id']);
                
                if (!in_array($rowCC['year_of_enrollment'], $s)) {
                $rows .= "
                <tr>
                    <td colspan='4' class='mt10 mb10 bgBlue'><b>{$rowCC['year_of_enrollment']}</b></td>
                </tr>                
                ";
                }
                ...
                $rows .= "
                <tr>
                    <input class='course_contact_id' type='hidden' name='course_contact_id[]' value='{$rowCC['course_contact_id']}'>
                    <td class='first_name' width='20%'>{$studentRec['first_name']}</td>
                    <td width='30%'>
                        <select name='site_id[]' class='fld_site_id'>
                            <option value=''>Please Select Branch</option>
                            {$dbUtil->getDropDownFromSQLCols2($db, $sqlSite, $site_id)}
                        </select>
                    </td>
                    <td width='20%'>{$courseRec['title']}</td>
                    <td width='30%' class='batch_id'>
                        <select name='batch_id[]' class='fld_batch_id'>
                            <option value=''>{$modulesArr['pms_batch']['title']}</option>
                        </select>
                    </td>
                </tr>
                <tr>
            ...
            while ($rowStudent = $db->sql_fetchrow($resultStudent)) {
                ...
                WHERE cc.contact_id = {$rowStudent['contact_id']}
                  AND cc.year_of_enrollment = {$current_year}
                ";
                $resultCC = $db->sql_query($sqlCC);
                ...
                $rows .= "
                <tr>
                    <input class='contact_id' type='hidden' name='contact_id[]' value='{$rowStudent['contact_id']}'>
                    <input class='course_id' type='hidden' name='course_id[]' value='{$rowCC['course_id']}'>
                    <td class='first_name' width='20%'>{$rowStudent['first_name']}</td>
                    <td width='30%'>
                        <select name='site_id[]' class='fld_site_id'>
                            <option value=''>Please Select Branch</option>
                            {$dbUtil->getDropDownFromSQLCols2($db, $sqlSite, $site_id)}
                    ...
                    <td width='20%'>{$rowCC['course_title']}</td>
                    <td width='30%' class='batch_id'>
                        <input type='hidden' name='contact_id[]' value='{$rowStudent['contact_id']}'>
                        <select name='batch_id[]' class='fld_batch_id'>
                            <option value=''>{$modulesArr['pms_batch']['title']}</option>
                        </select>
                    </td>
