    ...
    function getQuickSearch() {
        ...
        <td>
            <select name='order_type'>
                ...
                {$dbUtil->getDropDownFromSQLCols1($db, $sqlOrderType, $sqlOrderType)}
           </select>
        </td>
    ...
    **/
    ...
    function getInvoicePortalDisplayDetail($order_id){
        ...
        while ($rowInvoice = $db->sql_fetchrow($result)) {
            ...
            $rows .= "
            <tr>
                <td>INV - {$rowInvoice['invoice_code']}</td>
                <td>{$invoice_date}</td>
    ...
    function getReceiptPortalDisplay($order_id = ''){
        ...
        while ($rowReceipt = $db->sql_fetchrow($result)) {
            ...
            $rows .= "
            <tr>
                <td>RCPT - {$rowReceipt['receipt_code']}</td>
                <td>{$receipt_date}</td>
    ...
    function getPrintReceipt() {
        ...
        <table border="0" width="100%" style="font-size:15px;">
            <tr>
                <td align="center" style="font-weight:bold;"><u>RECEIPT</u></td>
            </tr>
        </table>
        ';
        ...
        $tbl2 ='<table border="0" width="100%" cellpadding="0" style="font-size:13px;">
                    <tr>
                        <td width="30%" style="line-height:20px;"><b>No : </b>'.$company['receipt_code'].'</td>
                        <td width="50%"></td>
                        <td width="20%" style="line-height:20px;" align="right"><b>Date : </b>'.$receipt_date.'</td>
                    </tr>
                </table>
                ';
        ...
        if($numRowsOrderItem > 0){
            ...
            $discount      = number_format($company['discount'], 2);
            $Total_Amount  = number_format($Total_Amount, 2);
            $total_in_words = $fn->getConvertNumber($company['receipt_amount'] .'.00');
        ...
        $tbl4 = '
        <table cellpadding="4" border="0" width="100%" style="font-size:13px;line-height:30px;">
            <tr>
                <td><i>Received with thanks from <u>'.$company['patient_name'].'('.$age .'/'.$gender.')</u> the sum of Rupees <u>'.$total_in_words.' towards '.$company['visit_procedure'].'</u> towards professional charges.</i></td>
            </tr>
        </table>
        ';
        ...
        <table cellpadding="4" border="0" width="100%">
            <tr>
                <td style="font-size:22px;"><img src="images/rupee.png" width="11" height="14"> <u>'.$ReceiptAmount.'</u></td>
                <td align="right" style="font-weight:bold;"><br/><br/>Signature</td>
            </tr>
        </table>
        ';
