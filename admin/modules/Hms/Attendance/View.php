...
class CPL_Admin_Modules_Hms_Attendance_View extends CP_Common_Lib_ModuleViewAbstract
{
    function getList($dataArray) {
        ...
        foreach ($dataArray as $row){
            ...
            if ($cpCfg['m.hms.attendance.hasMultipleSessions'] == 1) {
                ...
                {$listObj->getListDataCell($row['time_in_shift2'])}
                {$listObj->getListDataCell($row['leave_time_shift2'])}
                ";
            }
        ...
        if ($cpCfg['m.hms.attendance.hasMultipleSessions'] == 1) {
            ...
            {$listObj->getListHeaderCell('Day Time In', 'a.time_in')}
            {$listObj->getListHeaderCell('Day Time Out', 'a.leave_time')}
            ";

            $timeInOutHeaderNightShift = "
            {$listObj->getListHeaderCell('Night Time In', 'a.time_in_shift2')}
            {$listObj->getListHeaderCell('Night Time Out', 'a.leave_time_shift2')}
            ";
        }
        ...
        {$timeInOutHeaderDayShift}
        {$timeInOutHeaderNightShift}
        {$listObj->getListHeaderCell('Create By / Modified By')}
    ...
    function getNew(){
        ...
        $fieldset = "
        {$formObj->getDDRowBySQL($cpCfg['m.project.staffFieldLabel'], 'staff_id', $sqlStaff, $_SESSION['staff_id'])}
        ";
    ...
    function getEdit($row) {
        ...
        $time = "
        {$formObj->getTimeRow('Time in (HH:MM)', 'time_in', $row['time_in'], $expNoEdit)}
        {$formObj->getTimeRow('Time out (HH:MM)', 'leave_time', $row['leave_time'], $expNoEdit)}
        ";

        $timeNight = "
        {$formObj->getTimeRow('Time in (HH:MM)', 'time_in_shift2', $row['time_in_shift2'], $expNoEdit)}
        {$formObj->getTimeRow('Time out (HH:MM)', 'leave_time_shift2', $row['leave_time_shift2'], $expNoEdit)}
        ";
