...
class CPL_Admin_Modules_Core_Staff_Model extends CP_Admin_Modules_Core_Staff_Model
{
    ...
    function getImportData(){
        ...
        $fa = array(
             ...
             ,'user_group'  => $phpExcel->getImportFldObj('User Type')
             ,'user_name'   => $phpExcel->getImportFldObj('User Name')
             ...
             ,'site_id'     => $phpExcel->getImportFldObj('Location Id')
        );

        $fa['published']['defaultValue'] = 1;
        $fa['developer']['defaultValue'] = 0;
        $fa['status']['defaultValue']    = 'Current';
        $fa['user_group']['refOnly']     = true;
        $fa['site_id']['refOnly']        = true;
     ...
     *
     */
    function callbackAfterImportInsert($staff_id, $fa) {
        ...
        if ($fa['user_group'] != ''){
            ...
            FROM user_group
            WHERE title = '{$fa['user_group']}'
            ";
            $resultGrp = $db->sql_query($SqlUserGrp);
            $rowGrp    = $db->sql_fetchrow($resultGrp);

            $SqlUserGrpUpdate = "
            UPDATE `staff` SET  user_group_id = {$rowGrp['user_group_id']}, site_id = {$fa['site_id']}
            WHERE staff_id = '{$staff_id}'
            ";
            $resultGrpUpdate = $db->sql_query($SqlUserGrpUpdate);
        }
    }
