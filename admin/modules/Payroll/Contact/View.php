<?
class CPL_Admin_Modules_Payroll_Contact_View extends CP_Common_Lib_ModuleViewAbstract
{
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            if($cpCfg['m.payroll.contact.hasCompanyTable'] == 1){
                $rowsCompanyTable = "
                <td>
                    <div align='left'>
            ...
            if($cpCfg['m.payroll.contact.hasWebLogin'] == 1){
                $rowsWebLogin = "
                {$listObj->getListPublishedImage($row['published'], $row['contact_id'])}
                ";
            }

            if($cpCfg['m.payroll.contact.hasSalutation'] == 1){
                $rowsSalutation = "
        ...
        {$textSalutation}
        {$listObj->getListHeaderCell($ln->gd('m.payroll.header.contact.lbl.firstName', 'First Name'), 'c.first_name')}
        {$listObj->getListHeaderCell($ln->gd('m.payroll.header.contact.lbl.lastName', 'Last Name'), 'c.last_name')}
    ...
    function getNew(){
        ...
        $fielset = "
        {$formObj->getTBRow($ln->gd('m.payroll.contact.lbl.firstName', 'First Name'), 'first_name')}
        {$formObj->getTBRow($ln->gd('m.payroll.contact.lbl.lastName', 'Last Name'), 'last_name')}
    ...
    function getEdit($row){
        ...
        if($cpCfg['m.payroll.contact.showUsername'] == 1){
            $username = "
        ...
        $langPref = '';
        if ($cpCfg['m.payroll.contact.showLangPrefernce']){
    ...
    function getRightPanel($row){
        ...
        if($cpCfg['m.payroll.contact.showAttachment'] == 1){
            $rows .= $media->getRightPanelMediaDisplay($ln->gd('cp.lbl.attachment', 'Attachments'), 'payroll_contact', 'attachment', $row);
        }
        
        if($cpCfg['m.payroll.contact.showInterest'] == 1){
            $rows .= $displayLinkData->getLinkPortalMain("payroll_contact", "common_interestLink", "Interests Linked", $row);
        }

        if($cpCfg['m.payroll.contact.showEvent'] == 1){
            $rows .= $displayLinkData->getLinkPortalMain("payroll_contact", "event_eventLink", $ln->gd('m.payroll.contact.lbl.EventsLinked', 'Events Linked'), $row);
        }
        
        if($cpCfg['m.payroll.contact.showOrders']){
            $rows .= $displayLinkData->getLinkPortalMain("payroll_contact", "ecommerce_orderLink", "Orders Linked", $row);
        }

        if($cpCfg['m.payroll.contact.showContentLink'] == 1){
            $links .= $displayLinkData->getLinkPortalMain("payroll_contact", "webBasic_contentLink", "Content Linked", $row);
        }

        $record_id = $fn->getIssetParam($row, 'contact_id');

        $text = "
        {$media->getRightPanelMediaDisplay($ln->gd('cp.lbl.picture', 'Picture'), 'payroll_contact', 'picture', $row)}
        {$rows}
        {$comment->getView(array(
             'roomName' => 'payroll_contact'
            ,'recordId' => $record_id
        ))}
        {$links}
        ";
    ...
    function getQuickSearch() {
        ...
        if ($cpCfg['m.payroll.contact.showLangPrefernce'] == 1) {
            $languageText = "
            <td>
        ...
        //==================================================================//
        $spArray = $cpCfg['m.payroll.contact.specialSearchArr'];
    ...
    function getImportInstructions() {
        ...
        $url = 'index.php?_spAction=streamFile&showHTML=0&modname=payroll_contact&filename=contact-import-template.xls';
        $text = "
    ...
    function getAdditionalImportFields(){
        ...
        $text = '';
        if($cpCfg['m.payroll.contact.showInterestInImport']){
            $sqlInterest = $fn->getDDSql('common_interest');
