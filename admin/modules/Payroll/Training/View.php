<?
class CPL_Admin_Modules_Payroll_Training_View extends CP_Common_Lib_ModuleViewAbstract
{
    /**
     *
     */
                    ...
                    <table class='thinlist'>
                        <tbody>
                            <tr>
                                ...
                                <td>{$formObj->getDateRow('From date (YYYY-MM-DD)', 'date', $row['date'])}</td>
    ...
    */    
    function getEditold($row){
        ...
        {$formObj->getTBRow('Trainer', 'trainer', $row['trainer'])}
    ...
    function getRightPanel($row){
        ...
        if ($rowTraining['training_id'] != '') {
            $printText .="
            <div id='renewalLinkPortal'>{$this->getAddTrainingEmplyoee($row['training_id'])}</div>
            ";
        }
        $text=$text.$printText;
